...
viewof year_in = Inputs.input(5)
viewof i_in = Inputs.input(modelname == 'fizzbuzz' ? 0 : .04) // vs 2 for fizzbuzz!!
...
viewof term_in = Inputs.input(25)
...
mini_binds = {
  return ({
    ...
    year_in: Inputs.range([-1,40], {label:'year', step: 1}),
...
viewof overlay = Inputs.toggle({ label: "overlay?", value: false })
...
viewof formulae_visible = Inputs.input(formulae_all)// modelname == 'savings' ? ["balance", "interest", "deposits"] : modelname == 'fizzbuzz' ? formulae_all : [])
...
viewof ray_angle_step_size = Inputs.select([0.02,0.01,0.005], {label: 'ray angle step size'})
