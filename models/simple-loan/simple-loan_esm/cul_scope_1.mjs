import { d_i } from "./cul_scope_0.mjs";import { d_i_year } from "./cul_scope_0.mjs";import { skip_interest } from "./cul_scope_0.mjs";import { missed_repayment_year } from "./cul_scope_0.mjs";import { year } from "./cul_scope_0.mjs";import { term } from "./cul_scope_0.mjs";import { i } from "./cul_scope_0.mjs";import { principal } from "./cul_scope_0.mjs";import { interest_rate } from "./cul_scope_0.mjs";import { balance } from "./cul_scope_0.mjs";import { repayment } from "./cul_scope_0.mjs";import { repayment_due } from "./cul_scope_0.mjs";import { interest_repayment } from "./cul_scope_0.mjs";import { capital_repayment } from "./cul_scope_0.mjs";import { interest } from "./cul_scope_0.mjs";import { v_pow_term_left } from "./cul_scope_0.mjs";import { v } from "./cul_scope_0.mjs";import { pvifa } from "./cul_scope_0.mjs";import { repayment_amount } from "./cul_scope_0.mjs"; // automatic refinancing: an updated calc is applied every year
export const repayment_amount_ = ({ term_in, year_in, principal_in, d_i_year_in, i_in, d_i_in, missed_repayment_year_in, skip_interest_in }) => {
  if (term({ term_in }) == year({ year_in }) && Math.abs(balance({ year_in, principal_in, d_i_year_in, i_in, d_i_in, missed_repayment_year_in, skip_interest_in, term_in }) < 0.01)) return 0;else

  return (
    balance({ principal_in, d_i_year_in, i_in, d_i_in, missed_repayment_year_in, skip_interest_in, term_in, year_in: year({ year_in }) - 1 }) /
    pvifa({ term_in, year_in, d_i_year_in, i_in, d_i_in }));

};

// present value interest factor of annuity
export const pvifa_ = ({ term_in, year_in, d_i_year_in, i_in, d_i_in }) => {
  if (interet_rate() == 0) return term({ term_in }) - year({ year_in }) + 1;else
  return (1 - v_pow_term_left({ year_in, d_i_year_in, i_in, d_i_in, term_in })) / interest_rate({ year_in, d_i_year_in, i_in, d_i_in });
};

export const v_ = ({ year_in, d_i_year_in, i_in, d_i_in }) => 1 / (1 + interest_rate({ year_in, d_i_year_in, i_in, d_i_in }));
...
export const repayment_ = ({ year_in, missed_repayment_year_in, skip_interest_in, principal_in, d_i_year_in, i_in, d_i_in, term_in }) => {
  ...
  } else return repayment_due({ year_in, term_in }) * repayment_amount({ term_in, year_in, principal_in, d_i_year_in, i_in, d_i_in, missed_repayment_year_in, skip_interest_in });
};
